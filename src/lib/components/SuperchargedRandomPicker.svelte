<script lang="ts">
	...
	const genreFilterOptions = $derived(
		genreOptions.map((genre) => ({ value: genre, label: genre }))
	);

	const collectionFilterOptions = $derived([
		{ value: ALL_COLLECTIONS_VALUE, label: 'All collections' },
	...
	function handlePickFromFilters() {
		beginPick({ selectedGenres, selectedCollections });
	}
	...
	>
		...
		<div class="random-filter__panel surface-round anim-rise">
			...
			<div class="mt-5 space-y-4">
				{#if genreFilterOptions.length > 0}
					<div class="space-y-2">
						<p
						...
						<MediaFilterPopover
							...
							ariaLabel="Filter random pick by genre"
						/>
					</div>
				{/if}
				...
				{#if albums.length > 0}
					<div class="space-y-2">
						...
						<MediaFilterPopover
							...
							ariaLabel="Filter random pick by collection"
						/>
						<p class="text-xs font-medium text-[rgb(var(--color-text-tertiary))]">
							Leave empty for ungrouped movies only, or pick collections to widen the pool.
						</p>
					</div>
				{/if}
