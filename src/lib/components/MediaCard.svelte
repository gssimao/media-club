...
>
	<div class="relative aspect-[2/3] overflow-hidden bg-stone-100 dark:bg-stone-900">
		{#if item.listType === 'wishlist'}
			<div
				class="absolute top-2 left-2 z-10 flex items-center gap-1.5 rounded bg-amber-400 px-2.5 py-1 text-xs font-black tracking-wide text-stone-900 uppercase"
			>
				<span>Wishlist</span>
			</div>
		{/if}
			...
			<div class="mt-auto space-y-2 pt-2">
				{#if item.listType === 'wishlist'}
					<form method="POST" action="/admin/items?/moveToOwned">
						...
						<button
							type="submit"
							class="inline-flex w-full items-center justify-center gap-1.5 rounded border-2 border-amber-500 bg-amber-400 px-3 py-2 text-xs font-black tracking-wide text-stone-900 uppercase transition-colors hover:bg-amber-500"
						>
				...
				<div class="flex gap-2">
					{#if !editingNotes}
						<button
							...
							class="inline-flex flex-1 items-center justify-center gap-1 rounded border-2 border-stone-300 bg-white px-2.5 py-1.5 text-xs font-bold text-stone-700 uppercase transition-colors hover:border-stone-400 hover:bg-stone-50 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-300 dark:hover:border-stone-500 dark:hover:bg-stone-700"
						>
					...
					<form method="POST" action="/admin/items?/delete" class="flex-1">
						...
						<button
							type="submit"
							class="inline-flex w-full items-center justify-center gap-1 rounded border-2 border-red-400 bg-white px-2.5 py-1.5 text-xs font-bold text-red-600 uppercase transition-colors hover:bg-red-50 dark:border-red-700 dark:bg-stone-800 dark:text-red-400 dark:hover:bg-red-950"
						>
				...
				{#if editingNotes}
					...
					>
						...
						<div class="flex gap-2">
							<button
								type="submit"
								class="flex-1 rounded border-2 border-amber-500 bg-amber-400 px-3 py-1.5 text-xs font-bold text-stone-900 uppercase hover:bg-amber-500"
							>
								Save
							</button>
							<button
								...
								class="flex-1 rounded border-2 border-stone-300 bg-white px-3 py-1.5 text-xs font-bold text-stone-700 uppercase hover:bg-stone-50 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-300 dark:hover:bg-stone-700"
							>
								Cancel
							</button>
						</div>
